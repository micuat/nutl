...
function SYellowtail (p, w, h) {
  ...
  for (let i = 0; i < this.nGestures; i++) {
    this.gestureArray[i] = new Gesture(p.width, p.width);
  }
  this.clearGestures();
}

SYellowtail.prototype = Object.create(TLayer.prototype);
SYellowtail.prototype.constructor = SYellowtail;

SYellowtail.prototype.update = function () {
  this.updateGeometry();
}

SYellowtail.prototype.drawLayer = function(pg, key, args) {
	let p = this.p;
  pg.background(0, 0, 0);
  pg.noStroke();
  ...
  pg.fill(255, 255, 245);
  for (let i = 0; i < this.nGestures; i++) {
    this.renderGesture(pg, this.gestureArray[i], p.width, p.width);
  }
}
...
SYellowtail.prototype.renderGesture = function (pg, gesture, w, h) {
	let p = this.p;
  if (gesture.exists) {
    if (gesture.nPolys > 0) {
...
var s = function (p) {
  ...
  p.setup = function () {
    ...
    p.frameRate(60);
		...
		sYellowtail = new SYellowtail(p, 800, 1600);
		texes.push(sYellowtail);
		s051 = new S051(p, 800, 800, texes);
		s051.setup();
  }
