function S017 (p, w, h) {
  ...
  for(let i = 0; i < 500; i++) {
    ...
    Polygons.Hexagon(s, x, 50, z, w / Math.sqrt(3) * 0.9, 50);
    s.endShape(this.p.CLOSE);
    if(bloom) {
      this.shapeBloom.push({shape: s, phase: -radius * 0.01});
    }
    else {
  ...
  drawScene: {
    value: function (pg, isShadow) {
      ...
      if(!isShadow) {
        ...
        this.defaultShader.set("uSpecular", 0.2);
        // pg.shader(this.defaultShader);
      }

      pg.shape(this.shape, 0, 0);

      for(let i in this.shapeBloom) {
        let s = this.shapeBloom[i];
        if(!isShadow) {
          this.defaultShader.set("uMetallic", EasingFunctions.easeOutCubic(p.sin(p.millis() * 0.002 + s.phase * Math.PI * 2) * 0.5 + 0.5) * 0.9);
          this.defaultShader.set("uRoughness", 0.01);
          this.defaultShader.set("uSpecular", 0.1);
          // pg.shader(this.defaultShader);
        }
  
        pg.shape(s.shape, 0, 0);
      }
