...
S044.prototype.drawLayer = function(pg, key, args) {
  ...
  pg.pushMatrix();
  let y = EasingFunctions.easeInOutCubic((t*0.5)%1) * 100 * 5;
  pg.translate(0, y);
  ...
  for(let i = -15; i <= 5; i++) {
    for(let j = -5; j <= 5; j++) {
      ...
      if(sy >= -100 && sy <= pg.height + 100) {
        let noise = p.osnoise.eval(j*0.1, (-i + Math.floor(t*0.5)*5)*0.1);
        drawBar((-i + 4 + Math.floor(t*0.5)*5) % 4 + 1, -noise, false);
        drawCircle((-i + 5 + Math.floor(t*0.5)*5) % 4 + 1, noise, false);
      }
      pg.popMatrix();
    }
  }
